...
theorem k_axiom {atm : Type} (val : atm → W → Prop) {ϕ₁ ϕ₂ : @modal atm} {R : W → W → Prop} : ∀ (x : W), ϑ val R x (□(ϕ₁ => ϕ₂) => (□ϕ₁ => □ϕ₂)) :=
λ _ DAtoB DA y hy, DAtoB y hy (DA y hy)

def refl_axiom {R : W → W → Prop} : Prop := 
∀ {atm : Type} (val : atm → W → Prop) (ϕ : @modal atm) (x : W), ϑ val R x (□ϕ => ϕ)
...
∀ {atm : Type} (val : atm → W → Prop) (ϕ₁ ϕ₂ : @modal atm) (x : W), ϑ val R x (□(□ϕ₁ => ϕ₂) or □(□ϕ₂ => ϕ₁))
...
have H3 : (∀ (y_1 : W), (R y y_1) → (ϑ id R y_1 ϕ₁)), 
intros w hyw,
simp[ϕ₁],
from hyw,
have H4, from H2 H3,
...
have H3 : (∀ (y_1 : W), (R z y_1) → (ϑ id R y_1 ϕ₂)), 
intros w hzw,
simp[ϕ₂],
from hzw,
have H4, from H2 H3,
