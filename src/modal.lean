...
-- prefix `⊢`:20 := modal.prop -- A little unconventional, but I needed to have something here
notation `[`p`]` := modal.prop p
...
-- def testmodal {W : Type} (ϕ : W → Prop) (x : W) : modal W := ◇□◇⊢ϕ

@[simp] def modalprop {W : Type} (R : W → W → Prop) : W → modal W → Prop
| x (□ k) := ∀ y, (R x y) → @modalprop y k
| x (◇ k) := ∃ y, (R x y) ∧ modalprop y k
| x (k1 => k2) := modalprop x k1 → modalprop x k2
| x (neg k) := ¬ (modalprop x k)
| x (and k1 k2) := modalprop x k1 ∧ modalprop x k2
| x (or k1 k2) := modalprop x k1 ∨ modalprop x k2
| x [ϕ] := ϕ x
...
def eucl_axiom_2 {R : W → W → Prop} : Prop :=
∀ (ϕ : W → Prop) (x : W), R ϑ x (◇□[ϕ] => □[ϕ])
...
-- Frame conditions, corresponding to above axioms

def refl_fc {R : W → W → Prop} : Prop :=
∀ x : W, R x x

def trans_fc {R : W → W → Prop} : Prop :=
∀ x y z : W, R x y → R y z → R x z

def dense_fc {R : W → W → Prop} : Prop :=
∀ x z : W, R x z → ∃ y : W, (R x y) ∧ (R y z) 

def serial_fc {R : W → W → Prop} : Prop :=
∀ x : W, ∃ y : W, R x y

def symm_fc {R : W → W → Prop} : Prop :=
∀ x y : W, R x y → R y x

def eucl_fc {R : W → W → Prop} : Prop := 
∀ x y z : W, R x y ∧ R x z → R y z

def eq_fc {R : W → W → Prop} : Prop :=
∀ x y : W, R x y → x = y

def conv_fc {R : W → W → Prop} : Prop :=
∀ x y z, (R x y) ∧ (R x z) → ∃ w : W, (R y w) ∧ (R z w)

def h_fc {R : W → W → Prop} : Prop :=
∀ x y z, (R x y) ∧ (R x z) → (R y z) ∨ (R z y)

-- Proofs that axioms and frame conditions correspond

theorem refl_axiom_from_refl_fc {R : W → W → Prop} : @refl_fc W R → @refl_axiom W R :=
λ r_fc ϕ x sϕ, sϕ x (r_fc x)

theorem refl_fc_from_refl_axiom {R : W → W → Prop} : @refl_axiom W R → @refl_fc W R :=
λ r_ax x, (r_ax (λ u, R x u) x) (λ y hy, hy)

theorem trans_axiom_from_trans_fc {R : W → W → Prop} : @trans_fc W R → @trans_axiom W R :=
λ t_fc ϕ x sϕ y hxy z hyz, sϕ z (t_fc x y z hxy hyz)

theorem trans_fc_from_trans_axiom {R : W → W → Prop} : @trans_axiom W R → @trans_fc W R :=
λ t_ax x y z hxy hyz, (t_ax (λ u, R x u) x ) (λ _ a, a) y hxy z hyz

theorem dense_axiom_from_dense_fc {R : W → W → Prop} : @dense_fc W R → @dense_axiom W R :=
λ d_fc ϕ x sϕ z hxz, by
    rcases (d_fc x z hxz) with ⟨y, ⟨hxy, hyz⟩⟩;
    from sϕ y hxy z hyz

theorem dense_fc_from_dense_axiom {R : W → W → Prop} : @dense_axiom W R → @dense_fc W R :=
λ d_ax x z hxz, ((d_ax (λ z, ∃ y : W, (R x y) ∧ (R y z)) x) (λ y hxy z hyz, ⟨y, ⟨hxy, hyz⟩⟩)) z hxz

theorem serial_axiom_from_serial_fc {R : W → W → Prop} : @serial_fc W R → @serial_axiom W R :=
λ s_fc ϕ x sϕ, by
    cases s_fc x with y hxy;
    from ⟨y, ⟨hxy, sϕ y hxy⟩⟩

theorem serial_fc_from_serial_axiom {R : W → W → Prop} : @serial_axiom W R → @serial_fc W R :=
λ s_ax x,
begin
...
theorem symm_axiom_from_symm_fc {R : W → W → Prop} : @symm_fc W R → @symm_axiom W R :=
λ s_fc ϕ x h y hxy, ⟨x , ⟨s_fc x y hxy, h⟩⟩

theorem symm_fc_from_symm_axiom {R : W → W → Prop} : @symm_axiom W R → @symm_fc W R :=
λ s_ax x y hxy, by 
{have H, from (s_ax (λ y, x = y) x) (by simp) y hxy,
simp at H,
from H}

theorem eucl_axiom_from_eucl_fc {R : W → W → Prop} : @eucl_fc W R → @eucl_axiom W R :=
λ e_fc ϕ x dx, by
    rcases dx with ⟨y , ⟨hy, ϕy⟩⟩;
    from λ u hu, ⟨y, ⟨e_fc x u y ⟨hu,hy⟩,ϕy⟩⟩

theorem eucl_fc_from_eucl_axiom {R : W → W → Prop} : @eucl_axiom W R → @eucl_fc W R :=
λ e_ax x y z hxyz, by 
{have H, from (e_ax (λ y, y = z) x) (by simp; from hxyz.right),
simp at H,
from H y hxyz.left}

theorem eucl_axiom_2_from_eucl_fc {R : W → W → Prop} : @eucl_fc W R → @eucl_axiom_2 W R :=
λ e_fc ϕ x dsϕ, by
    rcases dsϕ with ⟨y,⟨hy, H1⟩⟩;
    from  λ z hz, H1 z (e_fc x y z ⟨hy, hz⟩)

theorem eucl_fc_from_eucl_axiom_2 {R : W → W → Prop} : @eucl_axiom_2 W R → @eucl_fc W R :=
...
theorem conv_axiom_from_conv_fc {R : W → W → Prop} : @conv_fc W R → @conv_axiom W R :=
begin
unfold conv_fc conv_axiom,
...
theorem conv_fc_from_conv_axiom {R : W → W → Prop} : @conv_axiom W R → @conv_fc W R :=
λ c_ax x y z hxyz,
begin
...
theorem h_fc_from_h_axiom {R : W → W → Prop} : @h_axiom W R → @h_fc W R :=
begin
unfold h_axiom h_fc,
